...
const settingsResolvers = {
  ...
  updateSettings: async (
    ...
    settings.queue_cleaner = args.settingsInput.queue_cleaner
    settings.content_search = args.settingsInput.content_search
    settings.failed_cleanup = args.settingsInput.failed_cleanup
    settings.library_cleanup = args.settingsInput.library_cleanup
    settings.permissions_change = args.settingsInput.permissions_change
    settings.storage_cleaner = args.settingsInput.storage_cleaner
    settings.tidy_directories = args.settingsInput.tidy_directories
    settings.queue_cleaner_loop = args.settingsInput.queue_cleaner_loop
    settings.content_search_loop = args.settingsInput.content_search_loop
    settings.failed_cleanup_loop = args.settingsInput.failed_cleanup_loop
    settings.library_cleanup_loop = args.settingsInput.library_cleanup_loop
    settings.library_cleanup_level = args.settingsInput.library_cleanup_level
    ...
    settings.storage_cleaner_loop = args.settingsInput.storage_cleaner_loop
  ...
  deleteUser: async (args: { userId: string }, req: AuthRequest): Promise<settingsType> => {
    if (!req.isAuth) {
      throw new Error("Unauthorised")
    }
  ...
  updateUserOverwrites: async (
    ...
    if (args.maxMoviesOverwrite !== undefined && args.maxMoviesOverwrite !== originalMovies) {
      ...
      const newDisplay =
        args.maxMoviesOverwrite === null ? "no overwrite" : args.maxMoviesOverwrite.toString()
    ...
    if (args.maxSeriesOverwrite !== undefined && args.maxSeriesOverwrite !== originalSeries) {
      ...
      const newDisplay =
        args.maxSeriesOverwrite === null ? "no overwrite" : args.maxSeriesOverwrite.toString()
